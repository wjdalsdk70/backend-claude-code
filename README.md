...
| 커맨드 | 설명 |
|--------|------|
| `/verify` | 빌드·테스트·커버리지·보안 전체 검증 |
| `/java-review` | Java/Spring Boot 코드 리뷰 |
| `/java-build` | 빌드 에러 진단 및 수정 |
| `/java-test` | TDD 워크플로우 |
| `/code-review` | 로컬 변경 또는 PR 종합 리뷰 |
| `/build-fix` | 빌드 에러 자동 수정 |
| `/db-migrate` | DB 마이그레이션 (Flyway/Liquibase) |
| `/test-coverage` | 커버리지 리포트 및 미달 영역 파악 |
| `/prp-plan` | PRP 계획 단계 |
| `/prp-implement` | PRP 구현 단계 |
| `/prp-commit` | PRP 커밋 단계 |
| `/prp-pr` | PR 자동 생성 (push → PR → CI 확인) |
| `/review-pr` | 다관점 PR 리뷰 |
| `/create-issue` | GitHub 이슈 생성 (bug / feat) |
