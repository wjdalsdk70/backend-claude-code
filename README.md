...
```bash
npx backend-claude-code@latest
```

특정 경로에 설치할 경우.

```bash
npx backend-claude-code@latest /path/to/your/project
```

재실행하면 업데이트됩니다. 소스에서 삭제된 파일은 자동으로 제거되고, 새 파일은 추가됩니다.

## 특정 파일 업데이트 제외

프로젝트에 맞게 커스텀한 파일은 `.claude/.claude-ignore`에 등록하면 업데이트 시 덮어쓰지 않습니다.
